...
export class EditIdeaComponent implements OnInit {
  ...
  editForm = this.fb.group({
    ...
    ideatype: [],
    interest: [null, [Validators.required, Validators.min(0), Validators.max(100)]],
    distribution: [null, [Validators.required, Validators.min(0), Validators.max(100)]],
  ...
  protected updateForm(idea: IIdea): void {
    this.editForm.patchValue({
      ...
      interest: idea.interest * 100,
      distribution: idea.distribution * 100,
      investment: idea.investment,
      active: idea.active,
      date: idea.date ? dayjs(idea.date).format(DATE_TIME_FORMAT) : null,
